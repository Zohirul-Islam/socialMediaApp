    ...
    <div className='flex flex-col h-screen'>
      ...
      <div className='px-4'>
              <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
                <input onKeyDown={(e)=>e.key === 'Enter' && sendMessage()} onChange={(e)=>setText(e.target.value)} value={text} type="text" className='flex-1 outline-0 text-slate-700' placeholder='type a message...' />
                <label htmlFor='image'>
                      {
                        image ? <img className='h-8 rounded' src={URL.createObjectURL(image)} alt="" /> : <ImageIcon className='size-7 text-gray-400 cursor-pointer'/>
                      }
